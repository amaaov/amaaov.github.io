...
Ensemble (panel E) layers up to four concurrent word loops, each with its own text/Morse, WPM, synth engine, mix/delay, and per-track MIDI (channel 1–16, note, velocity, optional program, MIDI enable separate from audio mute) into a dry master bus. Ensemble MIDI and the MIDI output tab share one Web MIDI port; Connect from either place. SAVE stores the current set (including MIDI params) as a local composition; the composition select swaps sets while keeping the master bus warm so a playing ensemble can continue without AudioContext restart lag. Toolbar Play / Loop / synth FX stay the single-output practice path; ensemble Play starts layers without replacing that path. Global Stop clears solo, ensemble, and keyed MIDI together.
