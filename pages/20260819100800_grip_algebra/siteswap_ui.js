...
const COPY = {
  en: {
    ...
    play: "Play",
    pause: "Pause",
    state: (state, held, airborne) => `${state}: ${held} held, ${airborne} airborne.`,
  ...
  ru: {
    ...
    play: "Пуск",
    pause: "Пауза",
    state: (state, held, airborne) => `${state}: удерживается ${held}, в полёте ${airborne}.`,
...
export function initializeSiteswapInterface(form, callbacks = {}) {
  ...
  function updateMetricReadout(row) {
    const values = {
      ...
      "metric-amphoteron": formatMetric(row.pAmphoteron),
