...
(function () {
  ...
  function refreshUI() {
    ...
    if (dockMeta && active) {
      var highLabel = highest ? highest.seal : "пока нет";
      ...
      dockMeta.setAttribute(
        ...
        active.done +
          " из " +
          active.total +
          " целей в активном грейде. Высшее непрерывное закрытие: " +
          highLabel +
          "."
      );
    }
    if (btnCert) {
      ...
      btnCert.textContent = highest ? "Журнал · " + highest.seal : "Журнал практики";
  ...
  var GRADE_NOTES = {
    ENT: {
      ...
      denom: "03",
      motto: "ПЕРВАЯ ОРБИТА",
      capability: "Главный каскад существует; устав принят.",
    ...
    "1A": {
      ...
      mark: "LUN",
      denom: "11",
      motto: "СЕРЕБРЯНЫЙ ПУТЬ",
      capability: "Фундаментальная тройка — выносливость и удержание плоскости.",
    ...
    "1B": {
      ...
      denom: "12",
      motto: "РОЗОВАЯ ПАУЗА",
      capability: "Каскад с доп. оборотом, бэккросс в передней плоскости, повороты с сохранением ориентации.",
    ...
    "1C": {
      ...
      mark: "MAR",
      denom: "13",
      motto: "ЖЕЛЕЗНЫЙ БРОСОК",
      capability: "Владение халфшауэром; освоение каскада вперёд; three-up 360.",
    ...
    "2A": {
      ...
      mark: "JUP",
      denom: "21",
      motto: "КОРОЛЕВСКИЙ КАСКАД",
      capability: "Каскад вперёд закреплён; естественный 180; четвёрка открыта.",
    ...
    "2B": {
      ...
      mark: "SAT",
      denom: "22",
      motto: "КОЛЬЦЕВОЙ ЗАКОН",
      capability: "Синхронный вперёд; скорректированные нечётные повороты; двойное доказательство перехода плоскости.",
    ...
    "2C": {
      ...
      denom: "23",
      motto: "БОКОВОЕ НЕБО",
      capability: "Длинные прогоны вперёд; цепочные связки; пируэты на четырёх.",
    ...
    "3A": {
      ...
      mark: "NEP",
      denom: "31",
      motto: "ГЛУБОКАЯ ПЛОСКОСТЬ",
      capability: "Выносливость четвёрки и глубина сайтсвопа; flash пятёрки с синхронным каскадом в передней плоскости 5.",
    ...
    "3B": {
      ...
      mark: "PLU",
      denom: "32",
      motto: "ПОДЗЕМНЫЙ ПРОХОД",
      capability: "Четвёрка вперёд открыта; выносливость пятёрки и халфшауэра наращены.",
    ...
    "3C": {
      ...
      mark: "SOL",
      denom: "33",
      motto: "СОЛНЕЧНАЯ ПЕЧАТЬ",
      capability: "Устойчивый контроль пятёрки; продвинутые связки на четырёх.",
    ...
    "4A": {
      ...
      denom: "41",
      motto: "СОЗВЕЗДИЕ",
      capability: "Длинные прогоны на пяти; скорректированные нечётные повороты на пяти.",
    ...
    "4B": {
      ...
      mark: "NOD",
      denom: "42",
      motto: "ВРАТА ЗАТМЕНИЯ",
      capability: "Пятёрка вперёд открыта; двойное доказательство перехода на пяти.",
    ...
    "4C": {
      ...
      mark: "COS",
      denom: "43",
      motto: "ПОЛНЫЙ НЕБОСВОД",
      capability: "Пиковая смешанная пятёрка; полное авторство переходов; длинные связки.",
  ...
  function astroForDate(dateStr) {
    ...
    return {
      ...
      label: sign.name + "  ·  " + moon.name + "  ·  день " + day.name,
    };
  }
  ...
  function paintCertificate(ctx, data, fmt) {
    ...
    var pathText = data.pathLabel ? "Путь  " + data.pathLabel : "Путь  " + data.seal;
    var recordText =
      data.gradeDone +
      "/" +
      data.gradeTotal +
      ...
      data.cumulTotal +
      " по пути";
    var metaText = "Записано  " + data.date + "  ·  " + note.motto + "  ·  " + data.gear;
    ...
    ctx.fillText("ИСТОРИЧЕСКАЯ ЗАПИСЬ ПРАКТИКИ  ·  v0.4", w / 2, y);
    ...
    ctx.fillStyle = accent;
    ctx.font = font("600", type.eyebrow, "'JetBrains Mono', monospace");
    ctx.fillText("УРОВЕНЬ ВЛАДЕНИЯ ПОИ", w / 2, y);
    y += lh.label + gap.tight;
    ...
    if (y < contentFloor - lh.name * 2) {
      ctx.fillStyle = mute;
      ctx.font = font("italic", type.body, "'Source Serif 4', Georgia, serif");
      ctx.fillText("высший закрытый грейд записан для", w / 2, y);
      y += lh.label + gap.tight;
    ...
    ctx.fillText("ПРОВЕРКА  " + serial, padX, sigY + 32 * s);
    ctx.textAlign = "right";
    ctx.fillStyle = mute;
    ctx.fillText(
    ...
    ctx.fillText(
      "Самоотслеживаемая историческая запись  ·  без peer-review  ·  CC BY 4.0",
      w / 2,
      hgt - padY + 16 * s
    );
  ...
  function gatherCertData() {
    ...
    var name = (certName && certName.value.trim()) || "Анонимный практикующий";
    var gear = (certGear && certGear.value.trim()) || "не указано";
    ...
    return {
      ...
      date: date,
      count: h.done,
      programme: "Квест навыков: жонглирование пои · Накопительный BN-устав · v0.4",
    };
  }
  ...
  function renderCertificate() {
    ...
    certCanvas.setAttribute(
      "aria-label",
      "Историческая запись практики для " +
        data.name +
        ", уровень владения пои " +
        data.seal +
        " — " +
        data.title +
        ", путь " +
  ...
  if (btnReset) {
    btnReset.addEventListener("click", function () {
      if (!window.confirm("Сбросить все отмеченные цели в этом браузере?")) return;
      boxes.forEach(function (box) {
        box.checked = false;
      });
      persist();
  ...
  function applyTermsMode(full) {
    ...
    if (termsLabel) {
      termsLabel.textContent = full
        ? termsLabel.getAttribute("data-when-on") || "Показать сокращения"
        : termsLabel.getAttribute("data-when-off") || "Показать полные имена";
    }
