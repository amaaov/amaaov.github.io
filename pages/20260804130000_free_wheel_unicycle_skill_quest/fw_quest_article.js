...
(function () {
  ...
  var PROGRAMME = "Free-Wheel Unicycle Skill Quest";
  var VERSION = "v0.1 · 4 August 2026";
  ...
  var CERT_FORMATS = {
    "landscape-16x9": { id: "landscape-16x9", label: "Landscape", w: 1920, h: 1080 },
    "classic-4x3": { id: "classic-4x3", label: "Classic", w: 1440, h: 1080 },
    "square-1x1": { id: "square-1x1", label: "Square", w: 1080, h: 1080 },
    "portrait-4x5": { id: "portrait-4x5", label: "Portrait", w: 1080, h: 1350 },
  };
  ...
  function updateUI() {
    ...
    grades.forEach(function (details) {
      ...
      if (prog) {
        if (s.id === "fw4") {
          prog.textContent = s.reqDone + "/" + s.reqTotal + " req · " + s.elDone + " elective";
        } else {
    ...
    if (dockGrade) {
      dockGrade.textContent = highest
        ...
        : "No riding grade sealed yet";
    }
    if (dockMeta) {
      dockMeta.textContent =
        overallDone + " / " + overallTotal + " tasks · " + VERSION;
    }
    if (dockFill && overallTotal) {
      ...
      if (dockBar) {
        ...
        dockBar.setAttribute("aria-valuetext", overallDone + " of " + overallTotal + " tasks");
      }
    }
    if (btnCert) {
      ...
      btnCert.setAttribute(
        "aria-label",
        unlocked
          ? "Open certificate for " + (highest ? highest.seal : "sealed grade")
          : "Certificate locked until a riding grade FW1–FW4 is sealed"
      );
    }
  }
  ...
  var GRADE_COSMOLOGY = {
    fw1: {
      seal: "FW1",
      body: "Mercury",
      rule: "Ignition at periapsis",
      motto: "FIRST BURN",
      capability: "Shoulders-forward start and stop, free coast",
    ...
    fw2: {
      seal: "FW2",
      body: "Venus",
      rule: "Brake scrub · coast freefall",
      motto: "DUAL PHASE",
      capability: "Shoulders-forward stops, pulse and float mixed",
    ...
    fw3: {
      seal: "FW3",
      body: "Earth–Moon",
      rule: "Bound long orbit",
      motto: "CONTINUOUS PATH",
      capability: "Distance outing, mounts, mild terrain",
    ...
    fw4: {
      seal: "FW4",
      body: "Mars",
      rule: "Eccentric ground · hop nodes",
      motto: "TERRAIN SEAL",
      capability: "Undulation, hop, long float, electives",
  ...
  function drawCertificate() {
    ...
    var cosmo = highest
      ...
      : GRADE_COSMOLOGY.fw1;
    var name = (certName && certName.value.trim()) || "Rider";
    var gear = (certGear && certGear.value.trim()) || "Hub / brake / wheel undeclared";
    ...
    drawMicroBand(
      ...
      H - inset - 16 * s,
      W - padX * 2,
      "FREEWHEEL QUEST · " + VERSION + " · " + date + " · INDEPENDENT OF IUF",
      metal,
      padX,
      s
    );
    ...
    ctx.fillText(
      highest ? cosmo.seal + " — " + highest.title : "No riding grade sealed",
      W / 2,
      H * 0.385
    );
  ...
  if (btnReset) {
    btnReset.addEventListener("click", function () {
      if (!window.confirm("Clear all checked tasks on this device?")) return;
      boxes.forEach(function (b) {
        b.checked = false;
      });
      persist();
